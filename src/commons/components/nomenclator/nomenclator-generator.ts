...
export const obtainInitialConsonantLength: any = (
  word: any,
  languageVocals: any
) => {
  for (let i = 0; i <= word.length; i++) {
...
export const sumAllChancesFromLetterGroupArray = (arr: any[]) =>
  arr.map((a: any) => a.chance).reduce((a, b) => a + b);

export const addNewLetterGroupToArray = (
  letterGroup: string,
  targetArray: LetterGroup[]
) => {
  const finalLetterGroup: LetterGroup = {
    group: letterGroup,
    chance: Object.values(targetArray).some((i) =>
      i.group.includes(letterGroup)
    )
      ? Object.values(targetArray).filter((i) =>
          i.group.includes(letterGroup)
        )[0].chance + 1
      : 1,
  };
  if (finalLetterGroup.chance > 1) {
    targetArray.splice(
      Object.values(targetArray).indexOf(
        Object.values(targetArray).filter((i) =>
          i.group.includes(letterGroup)
        )[0]
      ),
...
export const getInitialCharacters = (
  ...
  const generatewordWithLanguageFormattedData = (
    languageData: any,
    bodyLength: number
  ) => {
    const initialLetterAsVocalProbability = Math.floor(
      (languageData.initialVocals[languageData.initialVocals.length - 1].max *
        100) /
        (languageData.initialVocals[languageData.initialVocals.length - 1].max +
          languageData.initialConsonants[
            languageData.initialConsonants.length - 1
          ].max)
    );
    const finalLetterAsVocalProbability = Math.floor(
      (languageData.finalVocals[languageData.finalVocals.length - 1].max *
        100) /
        (languageData.finalVocals[languageData.finalVocals.length - 1].max +
          languageData.finalConsonants[languageData.finalConsonants.length - 1]
            .max)
    );
    let finalWord = "";
  ...
  const obtainComposedWord = (
    ...
    let finalWord = `${prefix}${generatewordWithLanguageFormattedData(
      frequencies,
      2
    )}`;
    if (interfix !== "") {
      ...
      if (randomComposendWord <= composedWordProbability) {
        for (let i = 1; i < composedWordMaxTerms; i++) {
          finalWord += ` ${generatewordWithLanguageFormattedData(
            frequencies,
            1
          )}`;
        }
  ...
  const generateManyNames = (namesNumber: number) => {
    ...
    for (let i = 0; i <= namesNumber; i++) {
      ...
      names = [
        ...names,
        obtainComposedWord(
          composedWordMaxTerms,
          composedWordProbability,
          frequency,
          interfix,
          prefix
        ),
